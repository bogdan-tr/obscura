...
public class GUI extends JFrame {
  ...
  private Image loadIcon() {
    try {
      ...
      // If icon.png doesn't exist, create a programmatic icon
      return createProgrammaticIcon(64);
    } catch (Exception e) {
      System.err.println("Warning: Could not load icon: " + e.getMessage());
      // Return a programmatic icon as fallback
      return createProgrammaticIcon(64);
    }
  }

  private Image createProgrammaticIcon(int size) {
    // Create an icon programmatically with gradient 'o'
    ...
    float outerRadius = (size - 2 * padding) / 2;
